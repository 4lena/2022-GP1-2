...
if ($files['Languages'] == 'English' && $flag == 0) {
    $command = escapeshellcmd('/Users/hourianalthunayangmail.com/opt/anaconda3/envs/spyder/bin/python API.py "' . $UploadedFile . '" "' . $fileID . '"');
    ...
    $command = escapeshellcmd('/Users/hourianalthunayangmail.com/opt/anaconda3/envs/spyder/bin/python APIar.py "' . $UploadedFile . '" "' . $fileID . '"');
    $output = shell_exec($command);
    $projectStatus = $collectionP->updateOne(
        ['ProjectName' => $ProjectName, 'username' => $username],
        ['$set' => ['ProjectStatus' => 'In Progress']]
...
foreach ($results as $commentInfo) {
    ...
    if ($idInfo == $fileID) { // if the comment from the same file 
        ...
        if ($scoreTOXICITY > 0.75)
            $toxic++; // increment toxic comments
...
$nonToxic = $numberOfResults - $scoreTOXICITY; // non toxic comments
if ($flag == 0) {
    $sumScores = $sumScores / $numberOfResults; // over all toxicty 
    $sumScores = $sumScores * 100; // over all toxicty percent 

    $project = $collectionP->findOne(['username' => $username, 'ProjectName' => $ProjectName]);
    $toxicityPro = $project['OverallToxicity'] + $sumScores; // retrive project's toxicty level
        ...
        <section class="section">
            <div class="row">
                <div class="col-lg-6">
                    <div class="card">
                        <div class="card-body">
                            <h5 class="card-title">Toxic Comments</h5>
                            <!-- Pie Chart -->
                            <div id="pieChart"></div>
                            ...
                            <script>
                                document.addEventListener("DOMContentLoaded", () => {
                                    new ApexCharts(document.querySelector("#pieChart"), {
                                        series: [<?php echo $scoreTOXICITY; ?>, <?php echo $nonToxic; ?>],
                                        chart: {
                                            height: 350,
                                            type: 'pie',
                                            toolbar: {
                ...
                if ($files['Languages'] == 'English') { // fain grain 
                    ...
                    echo '<div class="card">';
                    echo '<div class="card-body"><h5 class="card-title">Labels</h5>';
                    echo  '<div id="barChart"></div><script>'; //<!-- Bar Chart -->
