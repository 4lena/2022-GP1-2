...
<body>
  ...
  <aside id="sidebar" class="sidebar">
    <ul class="sidebar-nav" id="sidebar-nav">
      ...
      <li class="nav-item">
        <a class="nav-link collapsed" href="uploadComments.php?name=<?php echo $name; ?>">
      ...
      foreach ($results as $commentInfo) {
        ...
        if ($idInfo == $fileID) { // if the comment from the same file 
          ...
          if ($files['Languages'] == 'English') {
            ...
            echo '<h5 class="card-title">Labels</h5><div id="barChart' . $counter . '"></div><script>document.addEventListener("DOMContentLoaded", () => {';
            ...
            echo 'categories: ["INSULT", "THREAT", "PROFANITY", "SEVERE TOXICITY", "IDENTITY ATTACK"],}}).render();});</script></div></div></div>';
          } else
            echo '</div>';
        }
      }
